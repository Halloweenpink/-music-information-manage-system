...
class MusicAnalysisApp:
    def __init__(self, master):
        ...
        # 账号、密码与对应用户专属的excel信息表，用以匹配用户信息
        self.accounts = {
            # 用户密码+对应的路径地址
            "周鑫星": {"password": "123456", "excel_file": "all_person/zxx.xlsx"},
            "张倬玮": {"password": "123456", "excel_file": "all_person/zzw.xlsx"},
            "曹祥瑞": {"password": "123456", "excel_file": "all_person/cxr.xlsx"},
            "": {"password": "", "excel_file": "all_person/cxr.xlsx"},  # 开发者作弊路径，无需密码和用户名
            # "": {"password": "", "excel_file": "C:/Users/H16/Desktop/cxr.xlsx"},
        }
        # 歌曲总曲库
        self.songs_df = pd.read_excel(r"all_person/songs.xlsx")
        ...
        # 加载背景图片
        self.background_image = tk.PhotoImage(file='0.png')  # 更换为图片的实际路径，实现桌面壁纸切换
    ...
    def login(self):
        ...
        if username in self.accounts:
            ...
            # 账号密码不匹配则报错
                ...
                messagebox.showerror("Login Failed", "密码错误，请重新输入！！")
        else:
            messagebox.showerror("Login Failed", "未找到该用户账号！！")
    ...
    def delete_song(self):
        ...
        if song_name:
            if song_name in self.df['name'].values:
                ...
                self.df.to_excel(self.accounts[self.current_user]["excel_file"], index=False)
    ...
    # 加载数据的函数load_data
    def load_data(self):
        file_path = self.accounts[self.current_user]["excel_file"]
        self.df = pd.read_excel(file_path)
        # 从数据框中提取唯一的 'f1', 'f2', 'f3' 列中的元素，合并为一个集合，同时实现去重
    ...
    def show_today_recommend(self):
        ...
        # 去除重复项，确保关联标签是唯一的
        result = []
        merged_df = pd.concat([self.songs_df, self.df])
